...
app.get("/random",(req,res)=>{
  const a = Math.floor(Math.random()*jokes.length)
  res.json(jokes[a])
})
...
app.get("/filter", (req,res)=>{
  ...
  const joke = jokes.filter((jokes)=> jokes.jokeType == filter)
  res.json(joke)
})
...
//6. DELETE Specific joke
app.delete("/joke/:id",(req,res)=>{
  const id = parseInt(req.params.id);
  const joke = jokes.find((jokes)=>jokes.id === id);
  const index = jokes.indexOf(joke)
  if(joke){
    res.send(JSON.stringify(jokes.splice(index,1))+" is deleted")}
  else{
    res.send("The following joke doesnt exist")
  }})
